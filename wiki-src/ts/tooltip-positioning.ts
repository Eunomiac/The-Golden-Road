...
class TooltipFallbackController {
  ...
  constructor() {
    ...
    this.pairs.forEach((pair: TooltipPair): void => this.bindPair(pair));
  ...
  private applyVerticalFallback(pair: TooltipPair, tooltipHeight: number): void {
    ...
    const availableBelow = Math.max(
      window.innerHeight - anchorRect.bottom - clearance - padding,
      0
    );
   ...
   */
  private updateBoundaryOffsets(pair: TooltipPair, size: TooltipSize): void {
