 ...
 */
function initStatLabelAnimations(): void {
  const duration = 1.5;
  ...
  const statLabels: NodeListOf<Element> = document.querySelectorAll(
    ".stat-label.asset-tag, .stat-label.hypercompetent-tag"
  );
  ...
  const labelsWithIcons: Element[] = shuffleArray(Array.from(statLabels));
